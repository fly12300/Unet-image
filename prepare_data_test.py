        ...
        if os.path.exists(dataset_zip_dir)==True:
            ...
            for cvI in range(0,NumCV):
                ...
                if os.path.isdir(str(Path(crossvalid_dir,'CV'+str(cvI)))) == False:
                    ...
                    os.mkdir(str(Path(crossvalid_dir,'CV'+str(cvI),'train','full')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'train', 'image')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'train', 'full','image')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'train','label')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'train','full','label')))
                    os.mkdir(str(Path(crossvalid_dir,'CV' + str(cvI), 'test')))
                    os.mkdir(str(Path(crossvalid_dir,'CV' + str(cvI), 'test','full')))
                    ...
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'test','full', 'image')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'test','fullCT','label')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'test','full','label')))
                    ...
                    os.mkdir(str(Path(crossvalid_dir,'CV' + str(cvI), 'validate','full')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'validate', 'image')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'validate', 'full','image')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'validate','label')))
                    os.mkdir(str(Path(crossvalid_dir, 'CV' + str(cvI), 'validate','full','label')))
                ...
                for subItrain in range(int(numSubj / NumCV),len(subjectNums_cvI_trainVal)):  # take only the 3 folds for training
                    ...
                    for sliceI in range(0,sliceNums.size):
                        if NoHemorrhage[sliceI] == 0 or sliceI==randomSlice: # Saving only the windows that have hemorrhage or
                            ...
                            imsave(Path(crossvalid_dir, 'CV' + str(cvI),'train','full', 'image',str(sliceI) + '.png'), np.uint8(ct_scan[:, :, sliceI]))
                            imsave(Path(crossvalid_dir, 'CV' + str(cvI),'train','full', 'label',str(sliceI) + '.png'), np.uint8(masks[:, :, sliceI]))
                ...
                for subIvalidate in range(0, int(numSubj / NumCV)):  # take only the first fold for validation
                    ...
                    for sliceI in range(0,sliceNums.size):
                        if NoHemorrhage[sliceI] == 0: # Saving only the windows that have hemorrhage
                            ...
                            x_mask_segment = segment_ct(masks[:, :, sliceI], imageLen, windowLen, n_moves)
                            imsave(
                                Path(crossvalid_dir, 'CV' + str(cvI), 'validate', 'full', 'image', str(sliceI) + '.png'),
                                np.uint8(ct_scan[:, :, sliceI]))
                            imsave(
                                Path(crossvalid_dir, 'CV' + str(cvI), 'validate', 'full', 'label', str(sliceI) + '.png'),
                                np.uint8(masks[:, :, sliceI]))
                            #Saving only the windows that have hemorrhage
                            for i in range(n_moves*n_moves):
                                if sum(sum(x_mask_segment[:, :, i]))>0:
                ...
                for subItest in range(0, len(subjectNums_cvI_testing)):
                    ...
                    for sliceI in range(0, sliceNums.size):
                        ...
                        imsave(Path(crossvalid_dir, 'CV' + str(cvI),'test','full', 'image', str(sliceI) + '.png'), np.uint8(ct_scan[:, :, sliceI]))
                        imsave(Path(crossvalid_dir, 'CV' + str(cvI),'test','full', 'label', str(sliceI) + '.png'), np.uint8(masks[:, :, sliceI]))
