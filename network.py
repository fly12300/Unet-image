...
from model import jaccard_loss
...
def attention_up_and_concate(down_layer, layer, data_format='channels_first'):
    if data_format == 'channels_first':
        ...
        in_channel = down_layer.get_shape().as_list()[3]
    print("attention_up_and_concate ", data_format, in_channel)
    # up = Conv2DTranspose(out_channel, [2, 2], strides=[2, 2])(down_layer)
    up = UpSampling2D(size=(2, 2), data_format=data_format)(down_layer)
...
def unet2(img_w, img_h, n_label, data_format='channels_first',learningRate=1e-5,decayRate=1e-7):
    ...
    model.compile(optimizer=Adam(lr=learningRate, decay=decayRate), loss='binary_crossentropy',
                  metrics=['accuracy', jaccard_loss])
    #model.compile(optimizer=Adam(lr=1e-5), loss=[focal_loss()], metrics=['accuracy', dice_coef])
    return model
def unet1(pretrained_weights = None,input_size = (128,128,1),learningRate=1e-5,decayRate=1e-7, data_format='channels_first'):
    ...
    skips = []
    for i in range(depth):
        x = Conv2D(features, (3, 3), activation='relu', padding='same', kernel_initializer = 'he_normal')(x)
        #x = Dropout(0.2)(x)
        x = Conv2D(features, (3, 3), activation='relu', padding='same', kernel_initializer = 'he_normal')(x)
        skips.append(x)
        #x = MaxPooling2D((2, 2), data_format= data_format)(x)
    ...
    for i in reversed(range(depth)):
        ...
        print("UpSampling2D :",x)
        x = concatenate([x, skips[i]], axis=3)
        print("after concatenating", x)
...
########################################################################################################
#Attention U-Net
def att_unet(img_w, img_h, n_label, data_format='channels_first', learningRate=1e-5, decayRate=1e-7):
...
########################################################################################################
#Recurrent Residual Convolutional Neural Network based on U-Net (R2U-Net)
def r2_unet(img_w, img_h, n_label, data_format='channels_first', learningRate=1e-5, decayRate=1e-7):
    inputs = Input((img_w, img_h, 1))
    x = inputs
    depth = 4
    features = 64
...
########################################################################################################
#Attention R2U-Net
def att_r2_unet(img_w, img_h, n_label, data_format='channel_first', learningRate=1e-5, decayRate=1e-7):
    inputs = Input((img_w, img_h,1))
    x = inputs
    depth = 4
    features = 64
